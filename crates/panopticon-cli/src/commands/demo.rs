use anyhow::Result;
use panopticon_decomposition::{HybridStrategy, traits::DecompositionStrategy};
...
pub async fn run(state: &AppState) -> Result<()> {
    ...
    // --- 5. Permission check ---
    let approval =
        panopticon_permissions::ApprovalRequirement::from_characteristics(&task.characteristics);
